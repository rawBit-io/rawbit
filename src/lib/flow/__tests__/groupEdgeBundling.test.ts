  ...
  it("hides repeated cross-group edges and adds one bundle edge", () => {
    ...
    expect(bundle).toMatchObject({
      ...
      data: {
        ...
        sourceInsidePoint: { x: 290, y: 100 },
        sourcePoint: { x: 312, y: 100 },
        targetBoundaryPoint: { x: 500, y: 100 },
        targetInsidePoint: { x: 510, y: 100 },
        targetPoint: { x: 488, y: 100 },
      },
    });
    expect(bundle?.data?.sourceTerminals).toHaveLength(2);
    expect(bundle?.data?.targetTerminals).toHaveLength(2);
  });
  ...
  it("uses one fixed right output port and one fixed left input port per group", () => {
    ...
    expect(bundle).toMatchObject({
      data: {
        ...
        targetInsidePoint: { x: 10, y: 600 },
        targetPoint: { x: -12, y: 600 },
