# rawBit Lesson Summary

These notes summarize the 16 built-in lesson flows. Lessons 1-14 progress from legacy transaction construction through SegWit and Taproot, while lessons 15-16 add standalone labs for hardware signing and contributor practice. Each lesson shows the relevant bytes, signatures, scripts, and validation path.

- **Legacy foundations:** Lessons 1-7 cover P2PKH/P2PK, multisig, P2SH, timelocks, `OP_RETURN`, payment channels, and pre-SegWit malleability.
- **SegWit:** Lessons 8-10 cover witness serialization, BIP143 preimages, P2WPKH, P2WSH, wrapped SegWit, and the fee impact of moving signatures and scripts into witness data.
- **Taproot:** Lessons 11-14 cover key-path spends, script-path spends, `OP_CHECKSIGADD` multisig, NUMS internal keys, and MuSig2 key aggregation.
- **Wallet and contributor labs:** Lessons 15-16 cover Trezor hardware signing comparison, BIP39/BIP32 derivation, RFC6979 signing, and a Summer of Bitcoin proof-of-competence flow.
- **Debugger focus:** The flows are meant to be inspected step by step, especially where script execution, stack changes, sighash construction, and validation failures matter.
...
## Lesson 15: Trezor Signing Flow

- Derive a testnet P2PKH address from a BIP39 mnemonic and BIP32 path.
- Ask a connected Trezor for the same address and compare it with rawBit's derived address.
- Build a legacy P2PKH spend manually in rawBit using RFC6979/Trezor-style deterministic signing.
- Build Trezor Connect `signTransaction` parameters, sign on the device, and compare the serialized transaction with the manually signed rawBit version.
- Verify the final script execution so the hardware-signed transaction is tied back to the visible rawBit steps.

## Lesson 16: Summer of Bitcoin 2026 PoC

- Build a complete proof-of-competence flow for a Summer of Bitcoin 2026 student project.
- Create and fund a standard P2PKH address on testnet, then extract the funding transaction fields needed for spending.
- Spend the P2PKH UTXO into a P2SH output with a non-standard redeem script.
- Attempt to spend the P2SH output back to P2PKH and inspect why script verification can succeed while relay or broadcast policy may still reject the transaction.
- Add explanatory text nodes and a walkthrough so the flow demonstrates a full educational lesson, not just a partial transaction build.
